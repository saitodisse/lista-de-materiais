...
- A autorização Google usa `drive.file`, `openid` e `email`, sem incorporar escopos concedidos anteriormente (`include_granted_scopes: false`). Arquivos existentes precisam ser selecionados no Google Picker antes de vincular, inclusive quando a pessoa informa um link/ID. Tokens ficam somente em memória. A preferência de reconexão pode ficar no `localStorage`, mas nunca persista tokens, chaves da API, nem dados autenticados em URL, IndexedDB, logs ou service worker.
