...
- A autorização Google usa `drive`, `openid` e `email`, sem incorporar escopos concedidos anteriormente (`include_granted_scopes: false`). O vínculo aceita IDs, links do Drive e links completos do aplicativo e consulta o arquivo diretamente pela API. Tokens ficam somente em memória. A preferência de reconexão pode ficar no `localStorage`, mas nunca persista tokens, chaves da API, nem dados autenticados em URL, IndexedDB, logs ou service worker.
...
- Leitor e Editor são permissões do arquivo no Google Drive, não modos de URL. Preserve o bloqueio de envio quando `canModifyContent` for falso; leitura remota não impede a edição do catálogo local. O procedimento de compartilhamento e teste com duas contas está em `docs/compartilhamento-google-drive.md`.
